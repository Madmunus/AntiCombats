...
$S="select * from users where login='$param'";
$q=mysql_query($S);
...
$QUERY=mysql_query("SELECT * FROM users WHERE login='$param'");
$data=mysql_fetch_array($QUERY);

        $o_stat = $data['travm_old_stat'];
        $t_stat = $data['travm_stat'];
        $SQ = mysql_query("UPDATE users SET $t_stat='$o_stat',travm='0' WHERE login='$param'");
