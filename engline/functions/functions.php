...
class Test extends Error
{
	...
	function Up ()
	{
		...
		if ($add_bars)
		{
			...
			{
				$adb -> query ("UPDATE `character_bars` SET ?# = ?s WHERE `guid` = ?d", $value ,$bar_nums[$value]."|1" ,$this->guid);
			}
		}
		return;
	}
...
class Equip extends Error
{
	...
	function sellItem ($item_id)
	{
		...
		$db = $adb -> selectRow ("SELECT `money`, 
										 `money_euro` 
								  FROM `characters` 
								  WHERE `guid` = ?d", $this->guid) or $this -> Shop (203);
		$dat = $adb -> selectRow ("SELECT `c`.`id`, 
								   ...
								   WHERE `c`.`id` = ?d 
									 and `c`.`guid` = ?d 
									 and `c`.`wear` = '0' 
									 and `c`.`mailed` = '0';", $item_id ,$this->guid) or $this -> Shop (213);
		list ($item_id, $name, $item_type, $price, $price_euro) = array_values ($dat);
		if ($price > 0)
		{
			$price = $this -> getSellValue ($item_id);
			$new_m = $db['money'] + $price;
			$adb -> query ("UPDATE `characters` SET `money` = ?s WHERE `guid` = ?d", $new_m ,$this->guid);
			...
			$this -> Shop (404, "$name|$price");
		}
		else if ($price_euro > 0)
		{
			$price = $this -> getSellValue ($item_id, 'euro');
			$new_m = $db['money_euro'] + $price;
			$adb -> query ("UPDATE `characters` SET `money_euro` = ?s WHERE `guid` = ?d", $new_m ,$this->guid);
			$adb -> query ("DELETE FROM `character_inventory` WHERE `id` = ?d", $item_id);
			$history -> transfers ('Sell', "$name ($price екр)", $_SERVER['REMOTE_ADDR'], 'Shop');
			$this -> Shop (405, "$name|$price");
		}
	}
	...
	function showItemInventory ($item_info, $type, $i, $mail_guid = '')
	{
		...
		switch ($type)
		{
			...
			case 'sell':
				...
				$return .= "<img src='img/items/$img' border='0'$color /><br><center style='padding-top: 4px;'>";
				$return .= "<a href='main.php?sell=$item_id' onclick=\"if (confirm ('Вы уверены что хотите продать предмет $name за $s_price?')){return true;} else {return false;}\" class='nick'>продать за $s_price</a>";
			break;
			case 'mail_to':
				global $mail;
